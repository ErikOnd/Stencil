...
export function StencilApp({
	...
	function defaultValuesFor(target: PromptDraft) {
		return Object.fromEntries(
			target.variables.filter((variable) => variable.default).map((variable) => [variable.name, variable.default]),
		);
	}
	...
	function setFieldType(index: number, multiline: boolean) {
		setDraftState((current) => ({
			...
			variables: current.variables.map((variable, itemIndex) =>
				itemIndex === index ? { ...variable, multiline } : variable
			),
		}));
	}
	...
	async function copyFinal() {
		...
		if (useSourceId) {
			try {
				...
				setPrompts((current) =>
					current.map((prompt) => prompt.id === useSourceId ? { ...prompt, lastUsedAt } : prompt)
				);
			} catch {
				return;
			}
	...
	async function runImprove() {
		...
		try {
			...
			if (result.aiImprovedAt) {
				setPrompts((current) =>
					current.map((prompt) =>
						prompt.id === promptId ? { ...prompt, aiImprovedAt: result.aiImprovedAt ?? prompt.aiImprovedAt } : prompt
					)
				);
			}
			setAIOriginal(body);
