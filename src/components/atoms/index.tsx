...
export function Button(
	{ variant = "secondary", size = "medium", icon, loading, className, children, ...props }: ButtonProps,
) {
	return (
		<button
			className={clsx(buttonStyles.button, buttonStyles[variant], buttonStyles[size], className)}
			type="button"
			{...props}
		>
...
export function Textarea({ label, error, className, ...props }: TextareaProps) {
	return (
		<label className={fieldStyles.fieldWrap}>
			{label ? <span className={fieldStyles.label}>{label}</span> : null}
			<textarea
				className={clsx(fieldStyles.field, fieldStyles.textarea, error && fieldStyles.fieldError, className)}
				{...props}
			/>
			{error ? <span className={fieldStyles.errorText}>{error}</span> : null}
		</label>
...
export function Toggle(
	{ checked, onClick, "aria-label": label = "Toggle" }: {
		checked: boolean;
		onClick: () => void;
		"aria-label"?: string;
	},
	...
	return (
		<button
			className={clsx(toggleStyles.toggle, checked && toggleStyles.toggleOn)}
			type="button"
			onClick={onClick}
			aria-label={label}
		>
...
export function Checkbox(
	{ checked, onClick, className }: { checked: boolean; onClick: () => void; className?: string },
) {
	return (
		<button
			className={clsx(checkboxStyles.checkbox, checked && checkboxStyles.checkboxChecked, className)}
			type="button"
			onClick={onClick}
		>
			{checked ? "✓" : ""}
